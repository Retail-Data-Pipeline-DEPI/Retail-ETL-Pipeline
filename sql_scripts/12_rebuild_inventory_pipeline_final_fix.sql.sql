/*
=================================================================================
Script Name: Rebuild Inventory Pipeline (Final Harmonization & CRLF Fix)
Description: This script finalizes the inventory pipeline by addressing two issues:
             1. Harmonizes the branch name ('Alex') to match the CRM data.
             2. Fixes the CRLF issue in the 'Stock' column (the last column in the 
                CSV) to prevent stock levels from casting to zero.
             It then rebuilds the Gold Fact table with the fully cleansed data.
=================================================================================
*/
...
---------------------------------------------------------------------------
-- PART 1: Fix Zero Stock Issue & Branch Names in the Silver Layer
---------------------------------------------------------------------------
...
-- 1.3 Apply Data Harmonization and the CRLF Fix
WITH ConsolidatedInventory AS (
    -- Harmonize branch names to match sales data
...
DataQualityChecks AS (
    SELECT 
        ...
        -- The CRLF Fix: Remove hidden Carriage Return and Line Feed from the Stock column
        ISNULL(TRY_CAST(REPLACE(REPLACE(Stock, CHAR(13), ''), CHAR(10), '') AS INT), 0) AS Clean_Stock,
...
-- 1.4 Insert fully cleansed data into the Silver layer
...
---------------------------------------------------------------------------
-- PART 2: Rebuild the Gold Inventory Fact Table
---------------------------------------------------------------------------
...
-- 2.3 Populate the Fact table using Dimension lookups
...
-- 2.4 Data Validation: Verify that stock levels are correctly populated (not zeros)
